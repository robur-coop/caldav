...
sig 
  ...
  val read : t -> file -> (Cstruct.t * propmap, error) result Lwt.t
  ...
  val mkdir : t -> dir -> propmap -> (unit, write_error) result Lwt.t
  
  val write : t -> file -> Cstruct.t -> propmap -> (unit, write_error) result Lwt.t
